...
function createPerson(pos_x, pos_y, id, label) {
    createObjectBox(pos_x, pos_y, id, label);
    $("#" + id).data("PARTNER_RELID", '');
    $("#" + id).data("PARTNER_RELID", '');
    $("#" + id).data("HAS_RENDERED_CHILDREN", 'NO');
}

function createPartner(id, label, partner_id) {
    ...
    createObjectBox(rect.left + 290, rect.top, id, label);
...
function createChild(id, label, mother_id, father_id) {
    ...
    createObjectBox(last_x + 270, rect.top + 110, id, label);
